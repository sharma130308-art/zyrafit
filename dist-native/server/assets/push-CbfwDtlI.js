...
const VAPID_PUBLIC_KEY = "BN1t3lLwP1qB1UU-OlEyupcjA-JX2HPjaEAv3IjX3J9pCanVLsEGn3Wo7wB9DWkKqN0-Dt4Ls3AJZ-d4vVAxpC8";
...
function isPreviewEnvironment() {
  ...
  if (host.includes("id-preview--") || host.includes("lovableproject.com") || host === "localhost" || host === "127.0.0.1") {
    return true;
  }
  try {
...
async function subscribeToPush() {
  ...
  const { error: setErr } = await supabase.from("user_settings").update({ reminders_enabled: true, timezone }).eq("user_id", userId);
...
async function unsubscribeFromPush() {
  ...
  await supabase.from("user_settings").update({ reminders_enabled: false }).eq("user_id", userId);
  return { ok: true };
}
async function getRemindersEnabled() {
  const { data: userRes } = await supabase.auth.getUser();
  if (!userRes.user) return false;
  const { data } = await supabase.from("user_settings").select("reminders_enabled").eq("user_id", userRes.user.id).maybeSingle();
  return Boolean(data?.reminders_enabled);
}
export {
  isPreviewEnvironment as a,
  getRemindersEnabled as g,
  hasShownPrompt as h,
  isPushSupported as i,
  markPromptShown as m,
