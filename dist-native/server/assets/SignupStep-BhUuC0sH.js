...
import { i as isValidPhoneNumber, p as phoneToEmail, n as normalizePhone, l as lovable } from "./phone-auth-CMP2Nvmq.js";
...
function SignupStep({
  ...
  const handleSignUp = async (e) => {
    ...
    if (error) {
      setAuthError(
        error.message.toLowerCase().includes("already") ? "That number already has an account. Sign in instead." : error.message
      );
      setAuthLoading(false);
      return;
    }
    if (data.user) {
      ...
      await supabase.from("user_profiles").update({ phone: `+${normalizePhone(phone)}` }).eq("user_id", data.user.id);
      setAuthLoading(false);
      if (onComplete) {
        onComplete();
      } else {
  ...
  return /* @__PURE__ */ jsxs(
    StepContainer,
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("form", { onSubmit: handleSignUp, className: "space-y-4", children: [
          ...
          /* @__PURE__ */ jsx(
            motion.button,
            {
              ...
              children: authLoading ? /* @__PURE__ */ jsx(
                motion.div,
                {
        ...
        /* @__PURE__ */ jsxs("p", { className: "text-center text-sm text-muted-foreground mt-5", children: [
