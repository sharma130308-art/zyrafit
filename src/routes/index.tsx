...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f1020] text-white antialiased">
      {/* ───── Top nav ───── */}
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-[#0f1020]/70 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link
            to="/login"
            className="text-sm font-medium px-4 py-2 rounded-full bg-white/10 hover:bg-white/15 transition-colors"
          >
            Sign in
          </Link>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              ...
              <a
                href="#how"
                className="inline-flex items-center justify-center px-6 py-3.5 rounded-full bg-white/5 border border-white/10 text-white/80 font-medium hover:bg-white/10 transition-colors"
              >
                See how it works
              </a>
            </div>
      ...
      <section id="how" className="py-24">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <p className="text-xs uppercase tracking-[0.18em] text-primary/80 mb-3">How it works</p>
      ...
      <section id="features" className="py-24 bg-white/[0.02] border-y border-white/5">
        <div className="max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-14 items-center">
          <div className="order-2 lg:order-1">
            <p className="text-xs uppercase tracking-[0.18em] text-primary/80 mb-3">
              The killer feature
            </p>
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <div className="rounded-3xl bg-white/[0.04] border border-white/10 p-7 hover:bg-white/[0.06] transition-colors">
...
function ComparisonCard({
  ...
  return (
    ...
    >
      <h3 className={`font-semibold ${isUs ? "text-primary" : "text-white/70"}`}>{title}</h3>
      <ul className="mt-4 space-y-2.5">
        {points.map((p) => (
          <li key={p} className="flex items-start gap-2.5 text-sm">
            <span
              className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${
                isUs ? "bg-primary" : "bg-white/30"
              }`}
            />
            <span className={isUs ? "text-white" : "text-white/60"}>{p}</span>
          </li>
        ))}
      </ul>
    </div>
...
function Faq({ q, a }: { q: string; a: string }) {
  return (
    <details className="group rounded-2xl bg-white/[0.04] border border-white/10 px-5 py-4 open:bg-white/[0.06] transition-colors">
      ...
      <p className="mt-3 text-white/65 text-sm leading-relaxed">{a}</p>
    </details>
  );
}
