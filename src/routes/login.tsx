...
import { isNativeAppleAvailable, signInNativeApple, signInNativeGoogle } from "@/lib/native-auth";
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-6">
      ...
      >
        ...
        {(!isNative() || isNativeAppleAvailable()) && (
          <>
            ...
            <div className="flex gap-3">
              <motion.button
                ...
                onClick={async () => {
                  ...
                  if (isNative()) {
                    const result = await signInNativeGoogle();
                    setSocialLoading(null);
                    if (result.ok) {
                      await redirectAfterAuth(result.userId);
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-card border border-border/50 text-foreground font-medium hover:bg-accent transition-colors disabled:opacity-50"
              >
              ...
              <motion.button
                ...
                onClick={async () => {
                  ...
                  if (isNative()) {
                    const result = await signInNativeApple();
                    setSocialLoading(null);
                    if (result.ok) {
                      await redirectAfterAuth(result.userId);
                    } else if (result.error) {
